...
let rotate_right = function
    | Node (hc, key, value, x, rs) ->
      (
        match x with
          |Node (_, lkey, lvalue, lls, lrs) -> Node (hc, lkey, lvalue, lls,
...
let rotate_left = function
    | Node (hc, key, value, ls, x) ->
      (
        match x with
        | Node (_, rkey, rvalue, rls, rrs) -> Node (hc, rkey, rvalue,
...
let fixup h =
  ...
  let fix2 h = match h with
    | Node (_, _, _, Node (hlc, _, _, hll, _), _) ->
      (
...
let delete_min root =
  match root with
  | Leaf -> failwith "BST underflow"
  | Node (_, hkey, hvalue, hl, hr) ->
    (
      let red_root = Node (Red, hkey, hvalue, hl, hr) in
      let del_root = delete_min_aux red_root in
      match del_root with
      | Node (_, hkey, hvalue, hl, hr) -> Node (Black, hkey, hvalue, hl, hr)
      | _ -> Leaf
    )
...
let delete_max root =
  ...
  | Node (_, hkey, hvalue, hl, hr) ->
    (
      let red_root = Node (Red, hkey, hvalue, hl, hr) in
      let del_root = delete_max_aux red_root in
      match del_root with
      | Node (_, hkey, hvalue, hl, hr) -> Node (Black, hkey, hvalue, hl, hr)
      | _ -> Leaf
    )
...
let rec min_node = function
  | Node (_, _, _, Leaf, _) as h -> h
  | Node (_, _, _, hl, _) -> min_node hl
  | _ -> failwith "min_node"
...
let rec delete_aux node key =
  match node with
  | Node (_, hk, _, hl, hr) as h ->(
      if key < hk then
        let h' = match hl with Node (Red, _, _, hl, _) when is_red hl |> not -> move_red_left h | _ -> h in
        ...
        if key = hk && is_leaf hr then
          ...
          let h'' = match hr with
          | Node (Black, _, _, hl, _) -> if is_red hl |> not then move_red_right h' else h'
          | _ -> h' in
          match h'' with
          | Node (hc, hk, _, hl, hr) when key = hk ->
...
let delete root key  =
  let red_root = (match root with
  | Node (_, hkey, hvalue, hl, hr) ->
    if (is_red hl|> not) && (is_red hr |> not) then
      Node (Red, hkey, hvalue, hl, hr)
    else
      root
  | _ -> root) in
  let del_root = delete_aux red_root key in
  (match del_root with
  | Node (_, hkey, hvalue, hl, hr) -> Node (Black, hkey, hvalue, hl, hr)
  | _ -> Leaf)
