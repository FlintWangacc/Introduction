...
type vertex_info = { color : color ref; d : int ref ; p : vertex_info option ref; idx : int}
...
class graph (filename : string) =
    object (self)
        ...
        initializer
            ...
            vertexes <- Array.init
                            (num_vertices + 1)
                            (fun t -> { color = ref WHITE; d = ref max_int; p = ref None; idx = t})

        method private vertexes_to_int vv =
            Array.find_index (fun t -> t = vv) vertexes
        ...
        method private read_file2 (filename : string) =
            ...
            | hd:: tl ->
                (
                    ...
                    let _ = adjacency_list <- Array.make (n + 1) []; num_vertices <- n; is_directed <- directed in
        ...
        method bfs2 (s : int) =
            vertexes.(s) <- {color = ref GRAY; d = ref 0; p = ref None; idx = s};
            let rec bfs q =
                ...
                let q = Queue.create () in  let _ = Queue.add vertexes.(s) q in
                bfs q 
        ...
        method bfs (s : int)=
            ...
            | {color = color; d = d; p = p} as v ->
                ...
                | WHITE -> (
                    ...
                    v.d := !(u.d) + 1;
                    v.p := Some u;
                    Queue.add v bfs_queue
                )
            | _ -> () in
            let _ = Queue.add vertexes.(s) bfs_queue in
            while not (Queue.is_empty bfs_queue) do
                let u = Queue.take bfs_queue in
        ...
        method print_path (s : int) (v : int) =
            ...
            if v = s then
                ...
                | Some p -> let vpv = self#vertexes_to_int p |> Option.get in (self#print_path s vpv) @  [v]
...
let () =
    ...
    g#print_graph;
    g#bfs 1;
    List.iter (fun t -> Printf.printf "%d " t) (g#print_path 1 3); print_newline ();
